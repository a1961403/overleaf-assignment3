...
Figure 1 demonstrates the two RAG average scores of 10 queries. The answer relevancy and faithfulness score differences are very close that, faithfulness literly the same score, and the Naive RAG score in answer relevancy has only 0.06 higher than Hybrud RAG. This tells that the large model prompt engineering is completely success that the model is highly obedient to the instruction that answers can only be based on context. However, Naive RAG perform better in contextual Relevancy. It has more 0.1 score than Hybrid RAG, showing that the BM25 brings more noise the the system, causing the model generate inrrelevant answer.

Figure 2 shows both RAG scores in each queries for each metrics. For the Naive RAG, it has a perfect straight line in answer relevancy metric, showing that all the answer generated by LLM perfectly answer the question. It also perform well in faithfulness, it almost got another straight line in this metric. There are only two queries Q5 and Q7, both droped around 0.8 scores. The judge has given the reason that the answers were basically due to the evidence was not direct enough and the answer contained obvious wording errors. This could be caused by the integration capabilities of the model. However, there is a strong up and down fluctuations in contextual relevancy, although it has reached to score 1 in 1 query, another query has dropped to around 0.5. The judge has given the reason basicly saying that, there is a lot of noise, including irrelevant formatting and other irrelevant information, and the content is not focused enough. For the 
