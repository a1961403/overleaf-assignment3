...
Although this RAG very helpful on helping people to guilde people to use the cloud service, it will cause an outdate information risk. Cloud service update very frequent, if the database does not update at the same time, the system may suggest some secure configuration that already been disuse or contains vulnerability. Besides, the official document always suggest the best configuration the the user, without consider their budget. The system may ignore the needs of small and medium-sized enterprises with limited budgets or organizations with scarce resources, and recommend advanced secure service that is very high cost, such as advanced defender plan, leading to technology exclusivity.

\subsection{Transparency}

RAG successfully retrieve context from the official documents and provide to the LLM. However, the top-3 guidlines did not mark their reference source, which may reduced the trust among practitioners. Also, it is difficult for the enginner to verify how the AI hoose the 3 guildlines as the internal reasoning logic is hidden from the user when intergrating the documents.

\subsection{Governance \& Human-in-the-Loop}

To solve the issues mentioned above, it is recommended to create an automated knowledge update popline, allowing the system frequently extract the latest official documents from github, ensuring the timeliness. Besides, the disclaimer and risk warning should also included in the gerenated guildines, reminding the practitioner must have second evaluation on the guidlines based on their IT environment. Last but not least, the commands generated by the LLM must not authorization to execute automatically that need to always reviwed by a certified security experts before deploy to production environment.
